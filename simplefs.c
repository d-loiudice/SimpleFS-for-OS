			...
			if ( lseek(disk->fd, BLOCK_SIZE+disk->header->bitmap_bytes+disk->header->inodemap_bytes, SEEK_SET) != -1 )
			{
				...
				if ( read(disk->fd, inode, sizeof(Inode)) > 0 )
				{
					...
					if ( lseek(disk->fd, BLOCK_SIZE+disk->header->bitmap_bytes+disk->header->inodemap_bytes+(disk->header->inodemap_blocks*32), SEEK_SET) != -1 )
					{
						...
						{
							perror("Errore durante la lettura del primo blocco della directory root\n");
						}
					}
					else
					{
						perror("Errore durante lo spostamento per leggere il primo blocco della directory root\n");
					}
				}
				else
				{
					perror("Errore durante la lettura dell'inode principale (root)\n");
				}
			}
			else
			{
				perror("Errore durante il posizionamente per la lettura dell'inode principale (root)\n");
			}
		}
		free(bitmapInode);
	}
